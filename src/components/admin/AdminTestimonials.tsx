...
const AdminTestimonials = () => {
  ...
  const startEdit = (t: Testimonial) => {
    setEditingId(t.id);
    setForm({ quote: t.quote, name: t.name, title: t.title });
    setShowAdd(false);
  };

  const cancelEdit = () => { setEditingId(null); setForm({ quote: "", name: "", title: "" }); };

  const saveEdit = async () => {
    if (!editingId) return;
    const { error } = await supabase.from("testimonials").update({ quote: form.quote, name: form.name, title: form.title }).eq("id", editingId);
    if (error) toast({ title: "Update failed", description: error.message, variant: "destructive" });
    else { toast({ title: "Testimonial updated" }); cancelEdit(); fetch_(); }
  };

  const addNew = async () => {
    const { error } = await supabase.from("testimonials").insert({ quote: form.quote, name: form.name, title: form.title, display_order: testimonials.length });
    if (error) toast({ title: "Failed to add", description: error.message, variant: "destructive" });
    else { toast({ title: "Testimonial added" }); setShowAdd(false); setForm({ quote: "", name: "", title: "" }); fetch_(); }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        {!showAdd && <Button onClick={() => { cancelEdit(); setShowAdd(true); setForm({ quote: "", name: "", title: "" }); }} className="gap-2"><Plus size={14} />Add</Button>}
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-lg p-6 mb-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><Label className="text-xs">Title</Label><Input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="bg-secondary border-border" /></div>
            </div>
            <Button onClick={addNew} className="gap-2"><Save size={14} />Save</Button>
          </div>
        </motion.div>
      )}
